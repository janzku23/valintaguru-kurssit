    ...
    return Array.from(courseIds).map((courseId) => {
      ...
      const strongestArea =
        ([...enoughData].sort((a, b) => b.accuracy - a.accuracy || b.attempts - a.attempts)[0] ??
          [...rankedAreas].sort((a, b) => b.accuracy - a.accuracy)[0])?.area ?? null;
...
<section className="profile-grid" style={styles.grid}>
        ...
        <section className="profile-stats-grid" style={styles.statsGrid}>
          ...
          <StatCard
            ...
            value={formatProfileDuration(stats.averageAnswerTimeMs)}
          />
...
function formatProfileDuration(ms: number | null) {
  if (ms == null) return "-";
